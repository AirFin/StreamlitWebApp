...
st.markdown('# 0 简介')
st.markdown('爬取[东方财富股吧](https://guba.eastmoney.com/)中每个帖子的信息，并使用[snownlp](https://github.com/isnowfy/snownlp)对帖子标题进行情感分析，最后汇总为日数据进行数据展示。')

col1, col2 = st.columns(2)
with col1:
    asset_code = st.text_input('请输入资产代码', '605258')
with col2:
    pages = st.text_input('请输入要爬取的页数', '2')
